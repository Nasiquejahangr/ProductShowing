                ...
                <div className='row'>
                    {currentProducts.map((product) => (
                        <div key={product.id} className='col-md-4 my-2'>
                            <ProductItem title={product.title} description={product.description} price={product.price} image={product.images} category={product.category} discountPercentage={product.discountPercentage}/>
                        </div>
                    ))}
                </div>
                <div className='container d-flex justify-content-between my-5' style={{ marginBottom: "20px" }}>
                    <button disabled={currentPage === 1} onClick={prevPage} className='btn btn-primary'>Previous</button>
                    <button disabled={currentPage === Math.ceil(filteredProducts.length / productsPerPage)} onClick={nextPage} className='btn btn-primary'>Next</button>
                </div>
            </div>
        </>
    )
